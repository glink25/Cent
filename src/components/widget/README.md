...
Widget的功能入口位于settings页面-账本设置-主题与预设功能下方，名称为Widget设置，进入Widget设置页后，将会看到目前所有安装的Widget列表，列表中的元素包含Widget的名称、界面预览，和一个编辑按钮。Widget设置页面右上角有一个新增Widget按钮，点击按钮，或者点击列表Widget的编辑按钮，都会进入Widget编辑页面（新增的话即为空的widget），与其他设置功能的交互逻辑保持一致。（参考预算功能页）
