...
const filter = async (storeFullName: string, rule: BillFilter) => {
    const items = await getDB().getAllItems(storeFullName, true, ['time', 'desc']);
...
const getInfo = async (storeFullName: string) => {
    ...
    const globalMeta: GlobalMeta = await getDB().getMeta(storeFullName, undefined, true);
    ...
    return {
        ...
        categories: [
            ...Array.from(categoryIdsSet).map((id) => getDefaultCategoryById(id)).filter(v => v !== undefined),
